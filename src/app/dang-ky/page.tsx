...
export default function RegisterPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    setErrorMsg('');
    try {
      const redirectUrl = `${window.location.origin}/auth/callback?redirect=/ho-so`;
        ...
        <div className="p-8 sm:p-10 rounded-3xl bg-[var(--bg-card)]/80 border border-[var(--border-card)] backdrop-blur-2xl shadow-2xl space-y-8 relative overflow-hidden group">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative">
            ...
            <button 
              ...
              className="w-full rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold py-4 px-4 shadow-lg transition-all flex items-center justify-center mt-4 cursor-pointer"
            >
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-3 px-4 rounded-xl bg-white hover:bg-slate-50 text-slate-900 font-serif font-bold text-xs border border-slate-300 dark:border-slate-700 shadow-md transition-all flex items-center justify-center space-x-3 active:scale-[0.98] cursor-pointer disabled:opacity-50"
          >
